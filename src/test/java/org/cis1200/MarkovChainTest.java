...
public class MarkovChainTest {
    ...
    public void testAddSequenceEmpty() {
        MarkovChain mc = new MarkovChain();
        assertThrows(
                IllegalArgumentException.class, () -> mc.addSequence(null)
        );
    }

    @Test
    public void testAddSequenceReapeat() {
        MarkovChain mc = new MarkovChain();
        String sentence = "11 11";
        mc.addSequence(Arrays.stream(sentence.split(" ")).iterator());
        assertEquals(1, mc.bigramFrequencies.size());
        assertEquals(1, mc.bigramFrequencies.get("11").count(MarkovChain.END_TOKEN));
    }

    @Test
    public void testAddSequenceOneElement() {
        MarkovChain mc = new MarkovChain();
        String sentence = "11";
        mc.addSequence(Arrays.stream(sentence.split(" ")).iterator());
        assertEquals(1, mc.bigramFrequencies.size());
    ...
    @Test
    public void testAddEmptyIterator() {
        MarkovChain mc = new MarkovChain();
    ...
    @Test
    public void testWalkBigramNullString() {
        MarkovChain mc = new MarkovChain();
        assertThrows(
                IllegalArgumentException.class, () -> mc.addBigram("hello", null)
        );
        assertThrows(
                IllegalArgumentException.class, () -> mc.addBigram(null, null)
        );
        assertThrows(
                IllegalArgumentException.class, () -> mc.addBigram(null, "hello")
        );
    }

    @Test
    public void testWalkLastNextCheck() {
        String[] expectedWords = { "CIS", "1600" };
        MarkovChain mc = new MarkovChain();
